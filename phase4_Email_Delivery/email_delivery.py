...
class EmailDeliveryService:
    ...
    def __init__(self):
        ...
        self._note_service = None  # Lazy: avoid phase3->google.generativeai when sending from Gist
    ...
    def _get_note_service(self):
        """Lazy-load WeeklyNoteService only when path lookup is needed (not when content from Gist)."""
        if self._note_service is None:
            from phase3_Weekly_Note_Generation.note_generation import WeeklyNoteService
            self._note_service = WeeklyNoteService()
    ...
    def deliver_weekly_note(
        ...
        try:
            ...
            # Create response (message_id must be a string for Pydantic)
            response = EmailDeliveryResponse(
                message_id=delivery_result.get('message_id') or f"unknown_{int(time.time())}",
    ...
    def _get_latest_weekly_note_path(self) -> Optional[str]:
        ...
        try:
            latest_note = self._get_note_service().get_latest_weekly_note_file()
            if latest_note:
    ...
    def _create_email_message(
        ...
        try:
            ...
            # Create DOCX bytes for attachment: weekly pulse + fee explainer (so doc has both)
            docx_bytes = None
            if weekly_note_content_for_attach or (weekly_note_path and Path(weekly_note_path).exists() and weekly_note_path.endswith(".md")):
                ...
                if fee_section_plain:
                    content_for_docx = content_for_docx.rstrip() + "\n\n" + fee_section_plain
            ...
            text_body = PLAIN_TEXT_TEMPLATE.format(
                recipient_name=recipient_name,
                week_date=week_date,
                weekly_note_text=note_text + fee_section_plain,
            ...
            from phase4_Email_Delivery.config.email_templates import _escape_for_format
    ...
    def _markdown_to_docx(self, content: str) -> Optional[bytes]:
        """Convert markdown content to Word .docx bytes. Returns None if python-docx is not installed."""
        try:
            from docx import Document
        except ImportError:
            logger.warning("python-docx not installed; email will be sent without .docx attachment. Run: pip install python-docx")
            return None
        try:
            doc = Document()
