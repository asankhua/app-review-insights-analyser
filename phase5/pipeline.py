...
def get_email_preview() -> Optional[dict]:
    ...
    try:
        import re
        from phase4.config.email_templates import HTML_TEMPLATE, format_markdown_to_html, strip_markdown_headers
        ...
        raw_snippet = strip_markdown_headers(content).strip()[:250].replace("\n", " ")
        snippet = re.sub(r"\s+", " ", raw_snippet).strip()
        if len(strip_markdown_headers(content).strip()) > 250:
            snippet += "..."
